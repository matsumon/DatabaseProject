import { Table, Button, Typography, Input, Card, DatePicker, message } from 'antd';
...
function User() {
  ...
  const [rawData,setRawData]= useState([
    ...
    function createDataSource(){
      return _.map(rawData,(object,index)=>{
        return {
          ...
          <div>
            <Button 
              onClick={()=>{history.push(`/Sessions/${object.id}`)}}
            >
              Sessions
            </Button>
            <Button 
              onClick={()=>{history.push(`/Roles/${object.id}`)}}
            >
              Roles
            </Button>
            <Button 
              onClick={()=>{history.push(`/Actions/${object.id}`)}}
            >
              Available Actions
            </Button>
            <Button 
              onClick={()=>{history.push(`/Credentials/${object.id}`)}}
            >
              Credentials
            </Button>
            </div>
    ...
    function addUserToTable (){ 
      ...
      tempRawData.push({id: "",username: username,created:created, email:email});
      setUsername("")
  ...
  return (
      <div style={{ width: "100%", height: "100%" }}>
        ...
        >
          <p>
            Users can add users with a username, created date, and optional email
          </p>
          <p>
            Users can navigate to the users sessions, roles, available actions, and credentials.
          </p>
       </Card>
